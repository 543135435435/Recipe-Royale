...
import {
  ChevronRight, Flame, Users, BookOpen, Crown, Star,
  ArrowRight, Clock, TrendingUp, Globe, Utensils,
  Heart, Zap, Leaf, Award, MapPin, ChefHat, Sparkles,
  ArrowUpRight, Coffee, Cake, Pizza, UtensilsCrossed,
} from 'lucide-react';
...
import { CUISINES, CUISINE_IMAGES, CONTINENTS, CATEGORIES, FOOD_CATEGORY_IMAGES } from '../constants';
import {
  staggerContainer, staggerItem, heroReveal, heroWord,
...
export default function Home() {
  ...
  const { data: latestData } = useQuery({
    ...
    queryFn: () => recipeService.getRecipes({ limit: 8, sort: '-createdAt', status: 'published' }),
    staleTime: 5 * 60 * 1000,
  });
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        ...
        >
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            {/* Left: Text */}
            <motion.div variants={heroReveal} initial="hidden" animate="visible" className="space-y-7">
              <motion.div variants={heroWord} className="inline-flex items-center gap-2 px-4 py-2 bg-gold/10 rounded-full border border-gold/15">
                <Flame className="w-4 h-4 text-gold" />
                <span className="text-sm font-semibold text-gold tracking-wide">Premium Culinary Platform</span>
              </motion.div>

              <h1 className="text-5xl sm:text-6xl lg:text-[4.5rem] font-bold font-serif leading-[1.1] tracking-tight">
                {['Discover', 'the', 'World,', 'One', 'Recipe', 'at', 'a', 'Time.'].map((word, i) => (
                  <motion.span
                    key={i}
                    variants={heroWord}
                    className={`inline-block mr-3 ${
                      ['World,', 'Recipe'].includes(word) ? 'text-gradient-royal' : 'text-charcoal dark:text-white'
                    }`}
                  >
                    {word}
                  </motion.span>
              ...
              <motion.p variants={heroWord} className="text-lg sm:text-xl text-gray-500 dark:text-gray-400 max-w-xl leading-relaxed">
                Explore thousands of authentic recipes from cuisines around the globe. 
                Cook, share, and celebrate the art of food with a community that&apos;s passionate about flavor.
              </motion.p>
              ...
              {/* CTAs */}
              <motion.div variants={heroWord} className="flex flex-col sm:flex-row gap-4 pt-2">
              ...
              {/* Stats */}
              <motion.div variants={heroWord} className="flex items-center gap-8 pt-6">
                {[
                  { icon: <BookOpen className="w-5 h-5" />, value: '500+', label: 'Recipes', color: 'from-gold to-gold-light' },
                  { icon: <Users className="w-5 h-5" />, value: '50K+', label: 'Members', color: 'from-food-orange to-food-red' },
                  { icon: <Globe className="w-5 h-5" />, value: '35+', label: 'Countries', color: 'from-food-green to-food-emerald' },
                  { icon: <Star className="w-5 h-5" />, value: '4.9', label: 'Rating', color: 'from-gold to-food-orange' },
                ].map((stat, i) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    ...
                    <div>
                      <p className="text-lg font-bold text-charcoal dark:text-white leading-tight">{stat.value}</p>
            ...
            >
              {featuredRecipe && (
                <Link to={`/recipes/${featuredRecipe.id}`} className="block group">
                  <div className="relative rounded-3xl overflow-hidden shadow-royal-lg transform-gpu transition-transform duration-500 group-hover:scale-[1.02]">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-sm font-semibold text-gold">{featuredRecipe.cuisine}</span>
                        <span className="text-white/40">•</span>
                        <span className="text-sm text-white/70">{formatTime(featuredRecipe.totalTime)}</span>
                      </div>
            ...
            <div className="flex items-end justify-between mb-10">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-bold text-gold uppercase tracking-wider">Trending Now</span>
                </div>
                <h2 className="text-3xl lg:text-4xl font-bold font-serif text-charcoal dark:text-white">
                  Most Popular Recipes
                </h2>
              </div>
              <Link to="/recipes" className="hidden sm:flex items-center gap-1.5 text-gold hover:text-gold-dark font-medium text-sm transition-colors group">
        ...
        <div className="section-container">
          <AnimatedSection className="text-center mb-12">
            ...
            <p className="text-gray-500 max-w-lg mx-auto">
              From the streets of Lahore to the kitchens of Paris — discover flavors from every corner of the world
            </p>
          </AnimatedSection>
              ...
              <motion.div key={cuisine} variants={staggerItem}>
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 transform translate-y-1 group-hover:translate-y-0 transition-transform duration-300">
                    <h3 className="text-white font-bold text-base">{cuisine}</h3>
        ...
        <section className="py-20 lg:py-24 bg-white dark:bg-gray-900">
          <div className="section-container">
            ...
            <AnimatedSection direction="scale">
              <Link to={`/recipes/${featuredRecipe.id}`} className="group block">
                <div className="grid lg:grid-cols-2 gap-0 bg-cream dark:bg-gray-800 rounded-3xl overflow-hidden shadow-royal-lg">
                  ...
                  <div className="p-8 lg:p-12 flex flex-col justify-center">
                    <span className="text-sm font-semibold text-gold mb-2 tracking-wide uppercase">
                      {getCountryFlag(featuredRecipe.cuisine)} {featuredRecipe.cuisine}
                    </span>
                    ...
                    <div className="flex items-center gap-6 text-sm text-gray-500 mb-6">
                      <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-gold" /> {formatTime(featuredRecipe.totalTime)}</span>
                      <span className="flex items-center gap-1.5"><Star className="w-4 h-4 text-gold fill-gold" /> {featuredRecipe.rating?.toFixed(1) || 'New'}</span>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gold to-food-orange flex items-center justify-center text-white font-bold text-sm overflow-hidden ring-2 ring-white dark:ring-gray-800">
                          {featuredRecipe.author.avatar ? (
                            <img src={featuredRecipe.author.avatar} alt="" className="w-full h-full object-cover" />
                          ) : (
                            (featuredRecipe.author.firstName?.[0] || 'C').toUpperCase()
                          )}
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-charcoal dark:text-white">
                            {featuredRecipe.author.firstName} {featuredRecipe.author.lastName}
                          </p>
              ...
              <motion.div key={continent.name} variants={staggerItem}>
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    <h3 className="text-white font-bold text-lg mb-0.5">{continent.name}</h3>
                    <p className="text-white/60 text-xs line-clamp-1">{continent.description}</p>
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {continent.countries.slice(0, 3).map((c) => (
                        <span key={c} className="text-[10px] font-medium px-2 py-0.5 bg-white/15 rounded-full text-white/80">
                          {c}
                        </span>
                      ))}
      ...
      <section className="py-20 lg:py-24 bg-cream dark:bg-gray-950">
        <div className="section-container">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: <BookOpen className="w-7 h-7" />, title: 'Discover', desc: 'Browse hundreds of curated recipes from world-class chefs across every cuisine.', color: 'from-gold to-gold-light', emoji: '🔍' },
              { icon: <Flame className="w-7 h-7" />, title: 'Cook', desc: 'Follow detailed step-by-step instructions with timers and tips for perfect results.', color: 'from-food-orange to-food-red', emoji: '👨‍🍳' },
              { icon: <Users className="w-7 h-7" />, title: 'Share', desc: 'Share your own recipes and connect with passionate food lovers worldwide.', color: 'from-food-green to-emerald-500', emoji: '📸' },
              { icon: <Crown className="w-7 h-7" />, title: 'Crown', desc: 'Earn recognition, build your chef profile, and join the exclusive Crown Circle.', color: 'from-gold to-amber-500', emoji: '👑' },
            ].map((step, i) => (
              <AnimatedSection key={i} delay={i * 0.15}>
                <div className="relative bg-white dark:bg-gray-800 rounded-2xl p-8 text-center hover:shadow-card-hover transition-all duration-500 group border border-gray-50 dark:border-gray-700/50">
                  <div className={`w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br ${step.color} flex items-center justify-center text-white mb-6 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500 shadow-lg`}>
                    {step.icon}
                  </div>
                  ...
                  <h3 className="text-lg font-bold text-charcoal dark:text-white mb-2">{step.title}</h3>
      ...
      <section className="py-20 lg:py-24 bg-charcoal text-white relative overflow-hidden">
        ...
        <div className="section-container relative z-10">
          <AnimatedSection className="text-center mb-12">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold font-serif mb-4">Join the Crown Circle</h2>
            <p className="text-gray-400 max-w-xl mx-auto leading-relaxed">
              An exclusive community of passionate cooks. Share recipes, compete in challenges, and earn your place among the culinary elite.
            </p>
          </AnimatedSection>
            ...
            {[
              ...
              <AnimatedSection key={i} delay={i * 0.1}>
                <div className="text-center p-6 bg-white/5 rounded-2xl border border-white/10">
                  <p className="text-3xl lg:text-4xl font-bold text-gold mb-1">{stat.value}</p>
              ...
              <div className="flex items-end justify-between mb-10">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-bold text-food-green uppercase tracking-wider">Fresh & New</span>
                  </div>
                  <h2 className="text-3xl lg:text-4xl font-bold font-serif text-charcoal dark:text-white">
                    Recently Added
                  </h2>
      ...
      <section className="py-20 lg:py-24 bg-cream dark:bg-gray-950">
        <div className="section-container">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { name: 'Sarah M.', role: 'Home Chef', text: 'Recipe Royale transformed my cooking. The step-by-step instructions and timer features make complex recipes feel achievable.', avatar: 'SM', rating: 5 },
              { name: 'Ahmed K.', role: 'Food Blogger', text: 'As a Pakistani food blogger, I love sharing authentic recipes here. The Crown Circle community is incredibly supportive.', avatar: 'AK', rating: 5 },
              { name: 'Yuki T.', role: 'Professional Chef', text: 'The quality of recipes on this platform is unmatched. It\'s become my go-to for both inspiration and sharing my creations.', avatar: 'YT', rating: 5 },
            ].map((t, i) => (
              <AnimatedSection key={i} delay={i * 0.15}>
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 relative shadow-card hover:shadow-card-hover transition-shadow duration-300">
                  <div className="text-6xl text-gold/10 absolute top-3 left-5 font-serif leading-none">&ldquo;</div>
                  <div className="flex gap-0.5 mb-4">
                    {Array.from({ length: t.rating }).map((_, j) => (
                      <Star key={j} className="w-4 h-4 text-gold fill-gold" />
                    ))}
                  </div>
                  <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-6 pt-2">{t.text}</p>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-charcoal dark:text-white">{t.name}</p>
